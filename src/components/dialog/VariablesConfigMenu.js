...
const VariablesConfigMenu = ({ onPalsChange }) => {
  ...
  const handleSubmit = (e) => {
    ...
    return (
  <div className="bg-green-700 text-white p-4 rounded-lg shadow-md max-w-4xl mx-auto" style={scrollableStyle}>
    <h2 className="text-2xl font-bold mb-4">ToolBox - Game Settings - Pals Config</h2>
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6"> {/* Adjust grid for responsiveness */}
        {currentPals.map((pal) => (
            ...
            <div className="md:col-span-2 bg-green-800 p-4 rounded-lg">
              ...
              <div className="space-y-2">
                {Object.entries(pal.stats).map(([statName, value]) => (
                  <div key={statName} className="flex flex-col md:flex-row md:items-center">
                    <label className="block mb-1 capitalize font-medium">{statName}:</label>
                    <input
                      type="number"
                      value={value}
                      onChange={(e) => handleStatChange(pal.id, statName, parseInt(e.target.value, 10))}
            ...
            <div className="md:col-span-1 bg-green-800 p-4 rounded-lg">
              ...
              <ul className="space-y-1">
                {pal.moves.map((move, index) => (
                  <li key={index} className="font-medium">{move}</li>
                ))}
              </ul>
            </div>
          </React.Fragment>
